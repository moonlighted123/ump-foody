...
<body>
    <form method="post" name="frmQRGenerator" id="frm-qr"
        ...
        <div class="form-row">
            Email: <input type="text" name="qrid"
                id="qrid" class="input-field" />
        </div>
...
function validate() {
    ...
    if($("#qrid").val() == "") {
        	...
        	valid = false;
    } else if ( !( emailRegexp.test( $("#qrid").val() ) ) ) {
        $("#validation-info").show();
...
if (! empty($_POST["qrid"])) {
    ...
    $bobj = $barcode->getBarcodeObj('QRCODE,H', $_POST["qrid"], - 16, - 16, 'black', array(
        - 2,
        - 2,
        - 2,
        - 2
